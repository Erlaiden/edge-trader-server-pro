...
static void register_infer_routes(httplib::Server& srv){
  srv.Get("/api/infer", [](const httplib::Request& req, httplib::Response& res){
    ...
    if (!htf.empty()){
      ...
      json out{{"ok", inf.value("ok",false)},
               {"mode","pro"},
               {"symbol",symbol},
               {"interval",interval},
               {"version",version},
    ...
    // single TF
    json inf = etai::infer_with_threshold(M15, best_thr, ma_len);
    std::string sig = inf.value("signal","NEUTRAL");
    if (sig=="LONG") INFER_SIG_LONG.fetch_add(1, std::memory_order_relaxed);
    else if (sig=="SHORT") INFER_SIG_SHORT.fetch_add(1, std::memory_order_relaxed);
    else INFER_SIG_NEUTRAL.fetch_add(1, std::memory_order_relaxed);
    ...
    json out{{"ok", inf.value("ok",false)},
             {"mode","pro"},{"symbol",symbol},{"interval",interval},{"version",version},
             {"thr",best_thr},{"ma_len",ma_len},{"signal",sig},
             {"score",inf.value("score",0.0)},{"sigma",inf.value("sigma",0.0)},
             {"vol_threshold",inf.value("vol_threshold",0.0)}};
    res.set_content(out.dump(), "application/json");
  });
  ...
  srv.Get("/api/infer/batch", [](const httplib::Request& req, httplib::Response& res){
    ...
    const std::string path = "cache/models/" + symbol + "_" + interval + "_ppo_pro.json";
    std::ifstream f(path);
    if (!f){ json out{{"ok",false},{"error","model_not_found"},{"path",path}};
    ...
    std::string htf = qp(req, "htf", "");
    std::set<std::string> wanted;
    if (!htf.empty()){ std::string t; for(char c: htf){ if(c==','){ if(!t.empty()){wanted.insert(t); t.clear();} } else t.push_back(c);} if(!t.empty()) wanted.insert(t); }
