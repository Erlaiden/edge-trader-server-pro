// routes/pipeline.cpp
// Единый TU-роут "pipeline": backfill -> clean -> fill gaps (15m) -> train -> инфо для UI.
// Порт 3000, никаких новых сервисов. Использует существующие скрипты и эндпоинты.
// Авторежим без фантазий: JSON-in, JSON-out.
...
// безопасный запуск команды с захватом stdout
...
// true/false, есть ли дырки по 15м (шаг != 900000)
bool has_15m_gaps(const std::string& clean_path) {
    std::string awk = "awk -F, 'NR>1 {d=$1-prev; if(d!=900000) bad++; prev=$1} NR==1 {prev=$1} END{exit (bad?1:0)}' "
                      + clean_path;
    int rc = std::system(awk.c_str());
    // system() возвращает код в высоких битах; нормализуем:
    if (WIFEXITED(rc)) {
        int code = WEXITSTATUS(rc);
        return (code != 0);
    }
    // если что-то странное — считаем, что есть дырки, чтобы перестраховаться
    return true;
}

long rows_in_file(const std::string& path) {
    std::string cmd = "wc -l < " + path + " | tr -d ' \\n'";
...
// Публичная регистрация маршрутов
inline void register_pipeline_routes(httplib::Server& svr) {
    ...
    svr.Post("/api/pipeline/prepare_train", [](const httplib::Request& req, httplib::Response& res) {
        ...
        if (sym.empty()) {
            res.status = 400;
            resp["error"] = "missing_symbol";
            res.set_content(resp.dump(2), "application/json");
            return;
        }

        const int months   = in.value("months",   6);
        const int interval = in.value("interval", 15);  // пока пайплайн фиксирован на 15m для обучения
        ...
        // 2) Backfill через localhost:3000 (все ТФ)
        try {
            httplib::Client cli("127.0.0.1", 3000);
            cli.set_read_timeout(300, 0);
            std::string which = "15,60,240,1440";
            std::string path = "/api/backfill?symbol=" + sym + "&months=" + std::to_string(months) + "&which=" + which;
            ...
            json back_j = json::parse(r->body);
            step_ok("backfill", {{"intervals", back_j.value("intervals", json::array())}});
        ...
        // 3) CLEAN из RAW для всех ТФ
        {
            std::string cmd = "bash scripts/clean_from_raw.sh '" + sym + "'";
            std::string out = run_cmd(cmd);
            // лёгкая валидация по 15m
        ...
        // 4) Проверка и автозаполнение дыр 15m
        {
            const std::string clean15 = "cache/clean/" + sym + "_15.csv";
            bool gaps = has_15m_gaps(clean15);
            if (gaps) {
        ...
        // 5) Минимум 300 строк на 15m — обязательное условие
        {
            long r15 = rows_in_file("cache/clean/" + sym + "_15.csv");
            if (r15 < 300) {
        ...
        // 6) Обучение (fetch=0, cleanup=0, antimanip=1)
        json train_j;
        try {
            httplib::Client cli("127.0.0.1", 3000);
            cli.set_read_timeout(600, 0);
            std::ostringstream p;
            p << "/api/train?symbol=" << sym
              << "&interval=" << interval
              << "&episodes=" << episodes
              << "&tp=" << tp
              << "&sl=" << sl
            ...
            step_ok("train", {{"best_thr", train_j.value("best_thr", 0.0)},
        ...
        // 7) Быстрая сводка для UI: текущий сигнал + HTF
        json infer_j;
        try {
            httplib::Client cli("127.0.0.1", 3000);
            cli.set_read_timeout(60, 0);
            std::ostringstream p;
            p << "/api/infer?symbol=" << sym
              << "&interval=" << interval
              << "&htf=60,240,1440";
            auto r = cli.Get(p.str().c_str());
            if (r && r->status == 200) {
                infer_j = json::parse(r->body);
                step_ok("infer_snapshot", {{"signal", infer_j.value("signal", "NEUTRAL")},
