...
#include "train_logic.h"    // run_train_pro_and_save(...)
#include "rt_metrics.h"     // атомики
#include <ctime>

using json = nlohmann::json;

// Единая регистрация роутов тренировки.
// /api/train         — полная тренировка + сохранение модели (JSON на диск) + обновление атомиков
// /api/train/summary — укороченный ответ по ключевым метрикам (для UI)
static inline void register_train_routes(httplib::Server& srv) {
  // Полная тренировка и сохранение модели
  auto train_pro_handler = [](const httplib::Request& req, httplib::Response& res){
    ...
    // единая точка — сама пишет cache/models/*_ppo_pro.json и обновляет атомики
  ...
  // Короткое резюме (для UI) из того же запуска
  auto train_summary_handler = [](const httplib::Request& req, httplib::Response& res){
    ...
    // Достаём ключи для UI, где нужны только агрегаты
    ...
    json out{
      ...
      {"is",  {{"sharpe", is.value("sharpe", 0.0)}, {"expectancy", is.value("expectancy", 0.0)}}},
      {"oos", {{"sharpe", oos.value("sharpe", 0.0)}, {"expectancy", oos.value("expectancy", 0.0)}, {"drawdown_max", oos.value("drawdown_max", 0.0)}}},
