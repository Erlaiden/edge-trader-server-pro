...
static std::atomic<double> G_REWARD_WCTX{0.0}; // NEW

// Базовые коэффициенты формулы
...
// NEW: эффективные (динамические) коэффициенты — по умолчанию равны базовым
...
double get_reward_wctx()      { return G_REWARD_WCTX.load(); } // NEW
...
void   set_reward_wctx(double v)     { G_REWARD_WCTX.store(std::isfinite(v)?v:0.0); }   // NEW
...
void init_rewardv2_from_env(){
    ...
    // По умолчанию эффективные = базовые
    set_lambda_risk_eff(get_lambda_risk());
    set_mu_manip_eff(get_mu_manip());
}

// --- Эффективные коэффициенты ---
double get_lambda_risk_eff() { return G_LAMBDA_RISK_EFF.load(); }
double get_mu_manip_eff()    { return G_MU_MANIP_EFF.load(); }
void   set_lambda_risk_eff(double v) { G_LAMBDA_RISK_EFF.store(std::isfinite(v)?v:get_lambda_risk()); }
void   set_mu_manip_eff(double v)    { G_MU_MANIP_EFF.store(std::isfinite(v)?v:get_mu_manip()); }
